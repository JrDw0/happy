...
function AttachmentThumbnail({
    ...
    const isImage = image.kind === 'image' || (!image.kind && image.mimeType.startsWith('image/'));
    ...
    return (
        <View style={[
            ...
            {isImage ? (
                ...
                <View style={[styles.fileThumb, { backgroundColor: theme.colors.surfaceHigh }]}>
                    <Ionicons name={image.kind === 'video' ? 'videocam-outline' : 'document-text-outline'} size={24} color={theme.colors.text} />
                    <Text numberOfLines={2} style={[styles.fileName, { color: theme.colors.text }]}>{image.name}</Text>
                </View>
            )}
            {/* Remove button */}
            <Pressable
